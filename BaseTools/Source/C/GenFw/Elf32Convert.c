  ...
  )
{
  ...
  if (mExportFlag) {
    mCoffNbrSections ++;
    ElfFunctions->WriteExport = WriteExport32;
  }
  ...
  )
{
  ...
  AlignedOffset = (Offset + mCoffAlignment - 1) & ~(mCoffAlignment - 1);
  if (Offset != AlignedOffset) {
    mAlignmentDelta = mCoffAlignment;
  }
    ...
    for (i = 0; i < mEhdr->e_shnum; i++) {
      ...
      for (SymIndex = 0; SymIndex < SymNum; SymIndex++) {
        ...
        if (strcmp((CHAR8*)SymName, PRM_MODULE_EXPORT_DESCRIPTOR_NAME) == 0) {
          ...
          mExportRVA[mExportSymNum] = (UINT32)(Sym->st_value) + mAlignmentDelta;
  ...
  if(mExportFlag) {
    if ((mHiiRsrcOffset - mExportOffset) > 0) {
      CreateSectionHeader (".edata", mExportOffset, mHiiRsrcOffset - mExportOffset,
              EFI_IMAGE_SCN_CNT_INITIALIZED_DATA
              | EFI_IMAGE_SCN_MEM_READ);
      NtHdr->Pe32.OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_EXPORT].Size = mHiiRsrcOffset - mExportOffset;
      NtHdr->Pe32.OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress = mExportOffset;
  ...
  )
{
  ...
  NtHdr = (EFI_IMAGE_OPTIONAL_HEADER_UNION *)(mCoffFile + mNtHdrOffset);
  DataDir = &NtHdr->Pe32.OptionalHeader.DataDirectory[EFI_IMAGE_DIRECTORY_ENTRY_EXPORT];
